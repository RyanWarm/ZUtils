...
/** Private function used to implement get_terms() */
void
Xapian::Query::Internal::accumulate_terms_with_wdf(
			vector<pair<string, Xapian::termcount> > &terms) const
{
    if (is_leaf(op)) {
	// We're a leaf, so just return our term, but skip Query::MatchAllTerms
	// (which is Query("")).
	if (!tname.empty())
	    terms.push_back(make_pair(tname, parameter));
    } else {
	subquery_list::const_iterator end = subqs.end();
	// Not a leaf, concatenate results from all subqueries.
	for (subquery_list::const_iterator i = subqs.begin(); i != end; ++i) {
	    (*i)->accumulate_terms_with_wdf(terms);
	}
    }
}
...
std::map<string, Xapian::termcount> 
Xapian::Query::Internal::get_terms_with_wdf() const
{
    vector<pair<string, Xapian::termcount> > terms_vec;
    accumulate_terms_with_wdf(terms_vec);
    std::map<string,Xapian::termcount> terms_map; 
    for(size_t i=0;i<terms_vec.size();i++){
        if(terms_map.find(terms_vec[i].first)!=terms_map.end()){
            terms_map[terms_vec[i].first] += terms_vec[i].second;
        }else{
            terms_map[terms_vec[i].first] = terms_vec[i].second;
        }
    }

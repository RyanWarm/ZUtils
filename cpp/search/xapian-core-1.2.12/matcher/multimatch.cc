		     ...
		     const Xapian::KeyMaker *sorter)
{
    ...
    //std::cout<<__FILE__<<__LINE__<<":set vsdoc for postlists"<<std::endl;
    for(vector<PostList*>::iterator it=postlists.begin();it!=postlists.end();it++){
        (*it)->vsdoc(&vsdoc);
    }
	...
	if (sort_by != VAL || min_weight > 0.0) {
        //std::cout<<__FILE__<<" : "<<__LINE__<<" : postlist type = "<<typeid(*pl).name()<<std::endl;
        wt = pl->get_weight();
	    if (wt < min_weight) {

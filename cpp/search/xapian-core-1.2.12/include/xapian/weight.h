...
class XAPIAN_VISIBILITY_DEFAULT Weight {
  ...
  private:
    ...
    Xapian::docid docid_;
    Xapian::Database *database_;
    ValueStreamDocument *vsdoc_;
    std::string *term_;
  public: 
    
    ValueStreamDocument *vsdoc(ValueStreamDocument *vsd = 0);
    Xapian::docid current_docid(Xapian::docid docid = 0);
    std::string *current_term(const std::string *term = 0);
    Xapian::Database *database(Xapian::Database *db = 0);
